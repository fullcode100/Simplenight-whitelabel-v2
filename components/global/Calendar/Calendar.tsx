...
import { createCalendar, createNewMonth, DayObject, MonthObject } from '../../../helpers/calendar/calendar';
...
const DatePicker = ({
  ...
  const [calendar, setCalendar] = useState(createCalendar(initialYear, initialMonth));
  ...
  return (
    ...
    >
      ...
      />
      <section className='grid grid-cols-7 overflow-y-scroll text-center text-base items-center px-5' onScroll={handleScroll}>
        {calendar.map((month: MonthObject) => {
          return (
            <>
              ...
              <WeekDays />
              {month.days.map((day: DayObject) => (
                <Day
                  day={day}
                  setDate={setDate}
                  isStartDate={dayjs(day.date).isSame(dayjs(startDate))}
                  isEndDate={dayjs(day.date).isSame(dayjs(endDate))}
                  isRangeDate={dayjs(day.date).isBetween(dayjs(startDate), dayjs(endDate))}
                  isDisabled={dayjs(day.date).isSameOrBefore(dayjs().subtract(1, 'day'))}
                />
              ))}
            </>
          )
