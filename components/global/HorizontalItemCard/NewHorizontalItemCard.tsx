...
function HorizontalItemCard<T extends WithId>({
  ...
  return (
    <li
      key={item.id}
      className={`bg-white max-h-[250px] flex flex-col border border-dark-300 rounded-md ${className}`}
    >
      <Link href={url} passHref>
        <a target={target} rel="noopener noreferrer">
          <section className="flex ">
            {isImageComponent ? (
              <section className="min-w-[10%] max-h-[150px] lg:min-w-[15rem] lg:min-h-[11.3rem] flex justify-center items-center text-primary-1000">
                {ImageComponent}
              </section>
            ) : (
              <section
                className="min-w-[30%] max-h-[150px] lg:min-w-[15rem] lg:min-h-[11.3rem] "
                style={{
            ...
            <section className="flex flex-col justify-between pt-3 pl-4 lg:justify-start lg:w-full">
              <TitleSection />
