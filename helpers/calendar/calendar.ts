...
const createDaysForCurrentMonth = (year: number, month: number): MonthObject => {
  return { 
    yearNumber: year, 
    monthNumber: month, 
    monthName: dayjs(`${year}-${month}-01`).format("MMMM"), 
    days: [...Array(getNumberOfDaysInMonth(year, month))].map((_, index) => createDay(year, month, index)),
  };
}
...
export const createNewMonth = (calendar: Array<MonthObject>): MonthObject => {
  ...
  const newMonthDays = createDaysForCurrentMonth(lastMonthInCalendar.yearNumber, lastMonthInCalendar.monthNumber);

  return {...newMonthDays};
}

export const createCalendar = (year: number, month: number): Array<MonthObject> => {
  let calendar: any = [];
