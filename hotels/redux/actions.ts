...
const receiveHotels = (hotels: SearchItem[]) => ({
...
export const hotelsSetInitialState =
  (searchParams: HotelSearchRequest, i18next: i18n): AppThunk =>
  async (dispatch) => {
    dispatch(requestHotels());
    try {
      const { ClientSearcher: Searcher } = HotelCategory.core;
      const hotels: SearchItem[] = await Searcher?.request(
        searchParams,
        i18next,
      );
...
export const detail =
  (id: number, detailParams: HotelDetailPreRequest): AppThunk =>
  async (dispatch) => {
    try {
