...
export const TransportationItineraryBody: FC<
  TransportationItineraryBodyProps
> = ({ item, reload, setReload, breakdown = false }) => {
  ...
  const cancellationPolicyText = t(
        ...
        <section className="flex flex-col gap-3">
          ...
          <section className="flex items-center justify-between">
            <section className="flex items-center gap-2">
              ...
              <span className="font-semibold text-dark-1000 text-[16px] leading-[22px]">
                Point-To-Point Trip
              </span>
            </section>
...
const FeatureItem: FC<{
  icon: ReactNode;
  label?: string;
  labelWithTitle?: { title: string; label: string };
}> = ({ icon, label, labelWithTitle }) => {
  return (
    <section className="flex flex-row">
      ...
      {labelWithTitle && (
        <section className="flex flex-col gap-2">
          <span className="font-normal text-dark-700 text-[14px] leading-[17px]">
            {labelWithTitle.title}
          </span>
          <section className="font-semibold text-dark-1000 text-[16px] leading-[22px]">
            {labelWithTitle.label}
          </section>
        </section>
      )}
    </section>
...
const ItemDetails: FC<{ title: string; label?: string }> = ({
  ...
  return (
    <section className="flex flex-col gap-1">
      <span className="font-semibold text-dark-700 text-[16px] leading-[22px]">
        {title}
      </span>
      <p className="font-semibold text-dark-1000 text-[16px] leading-[22px]">
        {label}
      </p>
    </section>
  );
